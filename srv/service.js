...
class service extends cds.ApplicationService {
    ...
    async init() {
        ...
        this.before('PATCH', OrderItems, async req => {
            ...
            if (req.data.product_id != null && req.data.product_id != undefined) {
                productInfo = await cds.read(Products).byKey(product_id);
        ...
        this.after('PATCH', OrderItems , async req => {

            let orderInfo = { totamount: 0 };
            let result = await cds.read(OrderItems.drafts).where({ID : req.ID}).columns(['order_ID']);
